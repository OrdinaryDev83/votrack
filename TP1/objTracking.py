...
kf = KalmanFitler(dt=1, u_x=0, u_y=0, std_acc=0.1, x_sdt_meas=0.1, y_sdt_meas=0.1)
    ...
    if detected:
        for center in detected:
            ...
            to_show = cv2.rectangle(
                to_show,
                (int(updated_x - 15), int(updated_y - 15)),
                (int(updated_x + 15), int(updated_y + 15)),
                (0, 0, 255),
                2,
            )
            ...
            predicted_x, predicted_y = int(predicted_centroid[0]), int(
            ...
            to_show = cv2.rectangle(
                to_show,
                (int(predicted_x - 15), int(predicted_y - 15)),
                (int(predicted_x + 15), int(predicted_y + 15)),
                (255, 0, 0),
                2,
            )
            ...
            for j in range(1, len(trajectory)):
                if trajectory[j - 1] is None or trajectory[j] is None:
                    continue
                to_show = cv2.line(
                    to_show, trajectory[j - 1], trajectory[j], (0, 255, 255), 5
                )  # thickness is set to 5
    else:
        print("No detected")
